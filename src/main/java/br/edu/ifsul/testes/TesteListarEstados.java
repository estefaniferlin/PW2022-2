...
public class TesteListarEstados {
    ...
    public static void main(String[] args) {
        ...
        List<Estado> lista = em.createQuery("from Estado order by nome").getResultList();  // jpa faz um mapeamento objeto relacional, vou fazer uma consulta no meu objeto estado. Nao preciso fazer select com jpa, so comolocar um from da tabela (estado) e ele faz a pesquisa // Estado em maiúsculo porque ele pega o nome da Classe, e nao o nome que definimos para colocar la no banco
        
        for (Estado e : lista){   // para cada estado achado, mostra no console o id, nome e uf dele
